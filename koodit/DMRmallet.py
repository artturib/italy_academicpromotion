...
class DMRMallet(utils.SaveLoad):
    """
    Class for DMR training using MALLET. Communication between MALLET and Python
	...
	def fcovariatestxt(self):
		return self.prefix + 'covariates.txt'
		
	def fprogresstxt(self):
		return self.prefix + 'progress.txt'

    def fdatamallet(self):
        return self.prefix + 'data.mallet'

    def fwordweights(self):
        return self.prefix + 'wordweights.txt'

    def convert_input(self, corpus, covariates):
        """
		...
		covariates.to_csv(self.fcovariatestxt(), sep=' ',index=False,header=False) 
        # convert the text and covariate file above into MALLET's internal format
        cmd = ['cmd',self.mallet_path +'/bin/mallet/mallet.bat', 'run','cc.mallet.topics.tui.DMRLoader',str(self.fcorpustxt()),str(self.fcovariatestxt()),str(self.datamallet())]
        logger.info("converting temporary corpus to MALLET format with %s" % cmd)
        call(cmd)

    def train(self, corpus, covariates):
        self.convert_input(corpus, covariates)
        cmd = ['cmd',self.mallet_path +'/bin/mallet/mallet.bat','run','cc.mallet.topics.DMRTopicModel',str(self.datamallet()),str(self.num_topics)]
        # NOTE "--keep-sequence-bigrams" / "--use-ngrams true" poorer results + runs out of memory
        logger.info("training MALLET LDA with %s" % cmd)
        with codecs.open(self.progresstxt, 'w', encoding='utf-8') as progress_file:
			dmr_return = subprocess.call(cmd, stdout=progress_file)
